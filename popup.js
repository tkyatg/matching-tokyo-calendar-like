...
chrome.tabs.query( {active:true, currentWindow:true}, function(tabs){
  chrome.tabs.sendMessage(tabs[0].id, {message: 'isStarted' }, function(item){
    console.log(item)
    if(!item.stop){
...
$('#start-button').on('click', function(event){
  ...
  if(!enabled){
    ...
    chrome.tabs.query( {active:true, currentWindow:true}, function(tabs){
      chrome.tabs.sendMessage(tabs[0].id, {message: 'start' ,param}, function(item){
      });
    });
    enabled = true
    $('#start-button')[0].value = '停止'
    ...
    chrome.tabs.query( {active:true, currentWindow:true}, function(tabs){
      chrome.tabs.sendMessage(tabs[0].id, {message: 'stop'}, function(item){
      });
    });
    enabled = false
